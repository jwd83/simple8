// ============================================================================
//  Simple8MVBC — A tiny byte-addressable multicycle von Neumann CPU
//                with carry flag
//
//  Quick overview
//  ──────────────
//  • Extends Simple8MVB with a carry/borrow flag (C)
//  • ADD and SUB set both Z and C flags
//  • New instruction: JC — jump if carry
//  • All other instructions unchanged from Simple8MVB
...
//   0x0     NOP    do nothing              —
//   0x1     ADD    rd = rd + rs            Z, C
//   0x2     SUB    rd = rd - rs            Z, C
//   0x3     AND    rd = rd & rs            Z
//   0x4     OR     rd = rd | rs            Z
//   0x5     XOR    rd = rd ^ rs            Z
//   0x6     LDI    rd = imm8               Z
...
//  Flags
//  ──────────────────────────────────────────────────────
//   Z — set when result equals zero (ADD, SUB, AND, OR, XOR, LDI, LD)
//   C — set on unsigned overflow for ADD, unsigned borrow for SUB
//       (other instructions leave C unchanged)
//
//  Note: All addresses (LD, ST, JMP, JZ, JC) use the full 8-bit immediate,
//        giving access to the entire 256-byte address space.
//        Jump targets should be even addresses for valid instruction fetch.
//
// ============================================================================

module simple8mvbc_cpu (
    input  logic clk,
    input  logic reset
);
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            // ─────────────────────────────────────────────────────────
            //  Example program: demonstrates carry flag.
            //  Code starts at address 0x00 (occupies bytes 0x00–0x1F).
            ...
            // --- Carry flag demo: 200 + 100 = 300 → 44 with C=1 ---

            // LDI R0, 200        opcode=6, rd=0, rs=0 → high=0x60, low=0xC8
            mem[8'h14] <= 8'h60;  mem[8'h15] <= 8'hC8;

            // LDI R1, 100        opcode=6, rd=1, rs=0 → high=0x64, low=0x64
            mem[8'h16] <= 8'h64;  mem[8'h17] <= 8'h64;

            // ADD R0, R1         opcode=1, rd=0, rs=1 → high=0x11, low=0x00
            mem[8'h18] <= 8'h11;  mem[8'h19] <= 8'h00;

            // JC 0x1E            opcode=B, rd=0, rs=0 → high=0xB0, low=0x1E
            mem[8'h1A] <= 8'hB0;  mem[8'h1B] <= 8'h1E;

            // JMP 0x1C           opcode=9, rd=0, rs=0 → high=0x90, low=0x1C
            mem[8'h1C] <= 8'h90;  mem[8'h1D] <= 8'h1C;

            // NOP                opcode=0, rd=0, rs=0 → high=0x00, low=0x00
            mem[8'h1E] <= 8'h00;  mem[8'h1F] <= 8'h00;
            ...
            case (state)
                ...
                EXECUTE: begin
                    case (opcode)
                        ...
                        4'h1: begin // ADD rd, rs — sets Z and C
                            regfile[rd_idx] <= regfile[rd_idx] + regfile[rs_idx];
                            z_flag          <= ((regfile[rd_idx] + regfile[rs_idx]) == 8'd0);
                            c_flag          <= (({1'b0, regfile[rd_idx]} + {1'b0, regfile[rs_idx]})[8]);
                            pc              <= pc + 8'd2;
                            state           <= FETCH_HI;
                        end

                        4'h2: begin // SUB rd, rs — sets Z and C (borrow)
                            regfile[rd_idx] <= regfile[rd_idx] - regfile[rs_idx];
                            z_flag          <= ((regfile[rd_idx] - regfile[rs_idx]) == 8'd0);
                            c_flag          <= (regfile[rd_idx] < regfile[rs_idx]);
                            pc              <= pc + 8'd2;
                            state           <= FETCH_HI;
                        end
